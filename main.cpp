...
public:
  Tool(std::string name, std::string cat, std::string lnk, std::string desc)
      : toolName(name), category(cat), link(lnk), description(desc) {
  } // Modified constructor to include Tool Link
  ...
  void searchTools(const std::string &keyword) const {
    for (auto const &tool : tools) {
      if (tool.getToolName().find(keyword) != std::string::npos ||
          tool.getLink().find(keyword) !=
              std::string::npos || // Added the search capability for Tool Link
                                   // here
...
int main() {
  ...
  std::cout << "3. Write 'search' To Search Any Tool From Directory"
            << std::endl;
  ...
  while (true) {
    ...
    if (command == "add") {
      ...
      std::cout << "Invalid command. Try 'add', 'list', 'search', or 'exit'."
                << std::endl;
    }
  }
